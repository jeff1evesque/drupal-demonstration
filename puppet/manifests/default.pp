...
exec {'allow-htaccess-1':
    command => 'awk "/^(<Directory \/>|<\/Directory>)/{f=f?0:1}f&&/AllowOverride None/{$0=\"AllowOverride All\"}1" /etc/httpd/conf/httpd.conf > ~tmp.txt',
    refreshonly => true,
...
exec {'adjust-httpd-conf-1':
    command => 'mv ~tmp.txt /etc/httpd/conf/httpd.conf',
    refreshonly => true,
...
exec {'allow-htaccess-2':
    command => 'awk "/^(<Directory \"\/vagrant\">|<\/Directory>)/{f=f?0:1}f&&/AllowOverride None/{$0=\"AllowOverride All\"}1" /etc/httpd/conf/httpd.conf > ~tmp.txt',
    refreshonly => true,
