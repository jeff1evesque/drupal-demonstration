...
## allow htaccess (part 1): implement double backslash '\\' for literal escape
exec {'allow-htaccess-1':
    command => 'awk "/^(<Directory \\/>|<\\/Directory>)/{f=f?0:1}f&&/AllowOverride None/{\$0=\"    AllowOverride All\"}1" /etc/httpd/conf/httpd.conf > /etc/httpd/conf/httpd.conf',
    refreshonly => true,
...
exec {'allow-htaccess-2':
    command => 'awk "/^(<Directory \"\\/vagrant\\/">|<\\/Directory>)/{f=f?0:1}f&&/AllowOverride None/{\$0=\"    AllowOverride All\"}1" /etc/httpd/conf/httpd.conf > /etc/httpd/conf/httpd.conf',
    refreshonly => true,
